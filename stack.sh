...
GLANCE_HOST=${GLANCE_HOST:-$SERVICE_HOST}
# Glance connection info.  Note the port must be specified.
GLANCE_HOSTPORT=$GLANCE_HOST:$GLANCE_PORT

GLANCE_REGISTRY_HOST=${GLANCE_REGISTRY_HOST:-$SERVICE_HOST}
...
HORIZON_HOST=${HORIZON_HOST:-$SERVICE_HOST}
...
EOF
    ...
    mysql -u$MYSQL_USER -p$MYSQL_PASSWORD -h$MYSQL_HOST -e "DROP USER '$MYSQL_OPENSTACK_USER'@'$SERVICE_HOST';"
...
if is_service_enabled horizon; then
	...
	sudo sed -e "
		...
		s,%MYSQL_OPENSTACK_USER%,$MYSQL_OPENSTACK_USER,g;
...
if is_service_enabled g-reg; then
    ...
    iniset $GLANCE_REGISTRY_CONF DEFAULT bind_host $GLANCE_REGISTRY_HOST
    ...
    iniset $GLANCE_API_CONF DEFAULT bind_host $GLANCE_HOST
    iniset $GLANCE_API_CONF DEFAULT bind_port $GLANCE_PORT
...
if is_service_enabled m-svc; then
    ...
    if ! timeout $SERVICE_TIMEOUT sh -c "while ! http_proxy= wget -q -O- http://$M_HOST:$M_PORT; do sleep 1; done"; then
...
add_nova_opt "ec2_listen=$SERVICE_HOST"
...
screen_it horizon "cd $HORIZON_DIR && sudo tail -f /var/log/apache2/horizon/error.log"
